...
// ══════════════════════════════════════════════════════════════
//  FULLSCREEN SYSTEM — Animated enter/exit with close button
// ══════════════════════════════════════════════════════════════
...
function enterFullscreen(panel) {
    ...
    if (cy && panel === graphPanel) {
        setTimeout(() => { cy.resize(); cy.fit(null, 30); }, 100);
    }
}

function exitFullscreen(panel) {
    // Animate out
    panel.classList.add('panel-exiting-fullscreen');
    setTimeout(() => {
        panel.classList.remove('panel-fullscreen', 'panel-exiting-fullscreen');
...
// ══════════════════════════════════════════════════════════════
//  SIMULATION — Fullscreen graph → Fullscreen trace → Result
// ══════════════════════════════════════════════════════════════
...
async function runSimulation() {
    ...
    // ── Phase 1: Enter fullscreen for graph panel ──
    enterFullscreen(graphPanel);
    graphPanel.classList.add('sim-active-panel');

    // Wait for fullscreen transition then resize graph
    await sleep(500);
    ...
    // ── Phase 4: Enter fullscreen for output trace panel ──
    enterFullscreen(outputPanel);
    outputPanel.classList.add('sim-active-panel');
    await sleep(400);

    // ── Phase 5: Reveal trace steps one by one ──
    ...
    // Keep output highlighted briefly
    await sleep(800);
    outputPanel.classList.remove('sim-active-panel');

    // Don't auto-exit output fullscreen — let user read and close manually
    simulateBtn.disabled = false;
    simulateBtn.textContent = 'Simulate';
}

/** Cleanup helper if simulation is aborted mid-way */
function cleanupSimPanels() {
    graphPanel.classList.remove('sim-active-panel', 'panel-fullscreen');
    outputPanel.classList.remove('sim-active-panel', 'panel-fullscreen');

    // Remove close buttons
    [graphPanel, outputPanel].forEach(p => {
        const btn = p.querySelector('.close-fullscreen-btn');
        if (btn) btn.remove();
    });
...
function resetSimulation() {
    ...
    // Remove any fullscreen and active simulation highlights
    cleanupSimPanels();
}
...
function convertNfaToDfa() {
    ...
    // Auto-scroll to graph panel to show the new DFA graph
    showToast(`Converted NFA → DFA: ${newStates.length} states`, 'success');
    smoothScrollTo(graphPanel, 600);
}
...
function handleRegexConvert(target) {
    ...
    try {
        ...
        if (target === 'NFA') {
            ...
            // Auto-scroll to graph panel
            showToast(`NFA built: ${states.length} states, ${transitions.length} transitions`, 'success');
            smoothScrollTo(graphPanel, 600);
            ...
            // Auto-scroll to graph panel
            showToast(`DFA built: ${states.length} states, ${transitions.length} transitions`, 'success');
            smoothScrollTo(graphPanel, 600);
        }
...
function minimizeDFA() {
    ...
    // Auto-scroll to graph panel to show the minimized DFA
    showToast(`Minimized DFA: ${oldCount} → ${minStates.length} states`, 'success');
    smoothScrollTo(graphPanel, 600);
}
